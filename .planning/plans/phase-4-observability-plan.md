...
**Test run (repo state):** `python -m pytest agent/tests deploy/tests -q`; lint: `python -m ruff check agent`. Typical local run ~**139 passed**, ~**15 skipped** (approximate—counts vary). CI runs ruff + pytest on both test dirs (`.github/workflows/agent-tests.yml`, `.gitlab-ci.yml`). Instrumentation for verify/retry/degrade, misconfig (incl. client request id), category scaffold, RBAC minimum-question fields, **Prometheus counters** on `/agent/metrics` (incl. `openemr_misconfiguration_total`), and **minimum-question log contract tests** is in-tree; hosted dashboards / trace backends remain fork-optional.
