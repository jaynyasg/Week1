...
- **Agent tests (verified locally)**: `python -m pytest agent/tests deploy/tests -q` — typical **~139 passed**, **~15 skipped** (live OpenEMR, `RUN_LATENCY_GATE`, `RUN_LOAD_TEST`, etc.; run pytest to confirm). Optional: `-m "not eval"` deselects `@pytest.mark.eval` (e.g. latency module when enabled).
