...
write_copilot_apache_conf() {
...

  {
    ...
    echo "</Location>"
    echo ""
    echo "# Same proxy under /interface/ so browsers send OpenEMR session cookies whose"
    echo "# Path is limited to /interface (common when \$web_root is /interface)."
    echo "<Location /interface/clinical-agent>"
    echo "    ProxyPreserveHost Off"
    echo "    ProxyPass ${agent_url}/agent"
    echo "    ProxyPassReverse ${agent_url}/agent"
    echo "</Location>"
